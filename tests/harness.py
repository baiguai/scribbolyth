#!/usr/bin/env python3
"""
Reusable pty harness for scribbolyth integration tests.
...
relative to this file (../build/bin/scribbolyth), so cwd does not matter;
override it with the environment variable SCRIBBOLYTH_BIN.
"""
...
class Session:
    """A running scribbolyth instance inside a pseudo-terminal."""
...
def launch(bin_path=None, cols=80, rows=24, workdir=None):
    ...
    /tmp/scribbolyth_tests/ named after the test script (a previous run's
    files are removed). Pass an explicit workdir to keep it (e.g. to save
    a file and reload it in a second launch).
    """
    if bin_path is None:
        bin_path = os.environ.get('SCRIBBOLYTH_BIN') or DEFAULT_BIN
    if not os.path.exists(bin_path):
        print('FAIL: binary not found at %s' % bin_path)
        print('      set SCRIBBOLYTH_BIN or build the app first (./build.sh)')
        raise SystemExit(1)
    if workdir is None:
        script = os.path.basename(sys.argv[0] or 'test')
        name = os.path.splitext(script)[0]
        workdir = os.path.join('/tmp', 'scribbolyth_tests', name)
