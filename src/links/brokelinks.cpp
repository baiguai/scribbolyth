        ...
        void Recompute()
        {
            ...
            if (state_->collect_all_nodes)
            {
                for (const auto& item : state_->collect_all_nodes())
                {
                    const std::string key =
                        scribbolyth::links::Lower(scribbolyth::links::Trim(item.first->name));
                    if (!key.empty())
                    {
            ...
            if (state_->collect_all_nodes)
            {
                for (const auto& item : state_->collect_all_nodes())
                {
                    ...
                    std::vector<scribbolyth::links::Link> links;
                    scribbolyth::links::CollectLinks(node->text, links);
                    for (const auto& link : links)
                    {
                        if (!scribbolyth::links::IsNoteTarget(link.target)) continue;
                        const std::string title_key =
                            scribbolyth::links::Lower(scribbolyth::links::NoteTitle(link.target));
            ...
            {
                const std::string text = scribbolyth::links::NoteTitle(e.target)
                                         + "   (in " + e.node->name + ")";
