#!/bin/bash
#
# leakcheck.sh - run scribbolyth under valgrind and print a human-readable
...
#
# The raw valgrind log is kept at /tmp/scribbolyth-valgrind.log so you can
# dig deeper if the report flags anything.

source ./config.sh

BIN="./build/bin/$APP_NAME"
RAW="/tmp/scribbolyth-valgrind.log"
DOC="/tmp/scribbolyth-leak-doc.json"
LIVE=0
...
if [ "$real" -gt 0 ]; then
    ...
    echo "  harmless and not bugs in scribbolyth."
fi
